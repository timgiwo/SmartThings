...
definition(
    name: "Schedule Based Thermostat Control",
...
preferences {
    ...
    section("Time Slot") {
	    input "time1", "time", title: "At this time"
        input "heatingSetpoint1", "decimal", title: "When Heating"
        input "coolingSetpoint1", "decimal", title: "When Cooling"
    }
...
def initialize() {
    ...
    def scheduleTime1 = timeToday(time1, location.timeZone)
    def timeNow = now() + (2*1000) // ST platform has resolution of 1 minutes, so be safe and check for 2 minutes) 
    log.debug "Current time is ${(new Date(timeNow)).format("EEE MMM dd yyyy HH:mm z", location.timeZone)}, scheduled time is ${scheduleTime1.format("EEE MMM dd yyyy HH:mm z", location.timeZone)}"
    if (scheduleTime1.time < timeNow) { // If we have passed current time we're scheduled for next day
        log.debug "Current scheduling check time $scheduleTime1 has passed, scheduling check for tomorrow"
        scheduleTime1 = scheduleTime1 + 1 // Next day schedule
    }
    log.debug "Scheduling Temp change for " + scheduleTime1.format("EEE MMM dd yyyy HH:mm z", location.timeZone)
    schedule(scheduleTime1, setTheTemp)
}
...
def setTheTemp() {
    ...
    if(doChange == true){
        ...
        thermostat.setCoolingSetpoint(coolingSetpoint1)
        send "$thermostat heat set to '${heatingSetpoint1}' and cool to '${coolingSetpoint1}'"
    }
    else {
